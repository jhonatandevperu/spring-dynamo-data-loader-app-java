...
public class AwsConfig {
	...
	protected AmazonDynamoDB amazonDynamoDB() {
		...
		if (awsProperties.getEndpoint() != null) {
			EndpointConfiguration endpointConfiguration = new EndpointConfiguration(awsProperties.getEndpoint(),
					awsProperties.getDynamodb().getRegion() != null ? awsProperties.getDynamodb().getRegion() : null);

			return AmazonDynamoDBClientBuilder.standard().withCredentials(amazonAWSCredentialsProvider())
					.withEndpointConfiguration(endpointConfiguration).build();
		} else if (awsProperties.getDynamodb().getRegion() != null) {
			return AmazonDynamoDBClientBuilder.standard().withCredentials(amazonAWSCredentialsProvider())
					.withRegion(Regions.fromName(awsProperties.getDynamodb().getRegion())).build();
		} else {
	...
	protected AWSCredentialsProvider amazonAWSCredentialsProvider() {
		if (awsProperties.getAccessKey() != null && awsProperties.getSecretKey() != null) {
			...
			log.info("AWSCredentialsProvider access/secret keys: " + awsProperties.getAccessKey() + " , "
		...
		if (awsProperties.getRole() != null && awsProperties.getRole().getRoleArn() != null) {
			...
			log.info("AWSCredentialsProvider role/external ids: " + awsProperties.getRole().getRoleArn() + " , "
			...
			STSAssumeRoleSessionCredentialsProvider stsProvider = new STSAssumeRoleSessionCredentialsProvider.Builder(
					awsProperties.getRole().getRoleArn(), randomSesionName)
							.withExternalId(awsProperties.getRole().getExternalId()).withStsClient(stsClient)
							.withRoleSessionDurationSeconds(awsProperties.getRole().getTimeSession()).build();
